...
import { apiGetClassList } from '@/service/index'
// 查询参数
...
const searchData = async () => {
  try {
    ...
    if (queryParams.value.pageSize > res.data.list.length) noData.value = true
    if (res.data.list.length === 0 && classList.value.length === 0) noSearch.value = true
    console.log(res)
  } finally {
    uni.hideLoading()
  }
